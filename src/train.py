...
class MultiModel():
    ...
    def __init__(self) -> None:
        ...
        # Assuming self.X_train is already defined

        # Check if self.X_train is not empty
        if self.X_train is None or len(self.X_train) == 0:
            print("Error: self.X_train is empty or None.")
        else:
            # Check if self.X_train is numpy array or pandas DataFrame
            if not isinstance(self.X_train, (np.ndarray, pd.DataFrame)):
                print("Error: self.X_train should be a numpy array or pandas DataFrame.")
            else:
                # Check if self.X_train contains any missing values
                if self.X_train.empty:
                    print("Error: self.X_train contains missing values.")
                else:
                    # Check the shape of self.X_train
                    if self.X_train.ndim != 2:
                        print("Error: self.X_train should be a 2-dimensional array.")
                    else:
                        # Apply StandardScaler
                        sc = StandardScaler(with_mean=False)
                        from sklearn.feature_extraction.text import TfidfVectorizer
                        print('1:', self.X_train.shape, self.X_test.shape)

                        # Create TF-IDF vectorizer
                        tfidf_vectorizer = TfidfVectorizer()

                        # Fit TF-IDF vectorizer on training data and transform both training and testing data
                        self.X_train = tfidf_vectorizer.fit_transform(self.X_train['SentimentText'].tolist())
                        self.X_test = tfidf_vectorizer.transform(self.X_test['SentimentText'].tolist())

                        # Print shapes after transformation
                        print('2:', self.X_train.shape, self.X_test.shape)

                        # Now `X_train_tfidf` and `X_test_tfidf` contain TF-IDF features with the same vocabulary
        ...
        # from sklearn.feature_extraction.text import TfidfVectorizer
        # tfidf_vectorizer = TfidfVectorizer()
        # self.X_test = tfidf_vectorizer.fit_transform(self.X_test['SentimentText'].tolist())
